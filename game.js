...
var newBulletAi = function(vec) {
  return function(self, board, entities) {
    ...
    if (validPosition(board, self, newpos)) {
      ...
      for (i in entities) {
        if (entities[i] === self) {
          board.elementRef.removeChild(self.elementRef);
          entities.splice(i,1);
          break;
        }
      }
...
var elementFromEntity = function(entity) {
  var el = document.createElement("div");
  el.id = entity.id;
  el.className = "entity";
  el.style.width = entity.size + "px";
  el.style.height = entity.size + "px";
  el.style.backgroundColor = cssColor(entity.color);
  el.style.position = "absolute";
  if (entity.shape && entity.shape === "round") {
    el.style.borderRadius = (entity.size / 2).toFixed(0) + "px";
  }
  setPhysicalPosition(entity, el);
  return el;
};
...
var boardElement = function (board) {
  var el = document.createElement("div");
  el.id = board.id;
  el.className = "gameboard";
  el.style.width = board.x + "px";
  el.style.height = board.y + "px";
  el.style.position = "relative";
  el.style.backgroundColor = cssColor(board.background);
  return el;
};
...
var placeEntity = function(board, entity) {
  entity.elementRef = elementFromEntity(entity);
  board.elementRef.appendChild(entity.elementRef);
};
...
var loop = function(board, entities, keystate) {
  ...
  for (i in entities) {
    ...
    setPhysicalPosition(entity, entity.elementRef);
  }
